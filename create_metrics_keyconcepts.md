...
Before you start creating metrics, it helps to understand a few foundational concepts that guide how {{site.data.keyword.wxbia_short}} works. {: #shortdesc}
...
You can choose an existing project to work in or create a new project from the **Data and Metrics** tab. After you have identified a project to work in, select it from the project switcher on **Data and Metrics**. 
...
When you ask questions in **Conversations**, choose the project in the list in the input box. Watsonx BI uses the enriched metadata and metrics in that project to generate answers.
...
A semantic data model describes how data is structured, related, and calculated. It provides the meaning and context that is required for AI‑generated insights. For example, customer service data might include attributes such as Name, Location, and Product number, which can be linked to more product attributes like category, price, or color.
