...
- Valid access credentials (API key) 
  ...
  You must create a new API key. Do not use the API key that is automatically generated by watsonx BI.For information on how to create an API key, see [Managing user API keys](/docs/iam?topic=iam-userapikey&interface=ui){: external}. 
  {: important}
