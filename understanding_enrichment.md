...
Metadata enrichment is a critical step in watsonx BI that transforms raw data, which often lacks context, into a semantically rich dataset by using AI. This process adds business-relevant metadata such as names and descriptions, making the data easier to explore and understand. {: #shortdesc}
...
- Enrichment of metric definitions 
  ...
  When you create a metric in the semantic model, watsonx BI enriches it automatically. This enrichment helps ensure that new or modified metric definitions include clear and consistent metadata for conversational analysis.
...
Let's look at the differences between these enrichment types and how metadata is used by the conversational experience in watsonx BI. 

## Metadata enrichment of assets versus metrics 
{: #enrichment_assets_metrics}

Metadata enrichment takes place in two areas of watsonx BI. 

1. Enrichment of assets - takes place when you connect to or upload your data. Watsonx BI automatically begins the metadata enrichment process of the selected assets. 

2. Enrichment of metric definitions - takes place automatically when you generate metric definitions. If you create metrics definition manually, you need to export them to start metadata enrichment. 
...
During this process, watsonx BI uses AI to automatically generate business-friendly names and descriptions for these base assets. 

If you are using watsonx.data intelligence for enrichment, you can review, accept, or edit the AI-generated suggestions before they’re applied to the semantic data model.
...
Since enriched base assets form the foundation for the semantic data model and derived metrics, review the metadata enrichment results to ensure the following metadata is accurate and meaningful:
...
Metadata such as the column identifier, column data type, usage, aggregate, and nullable fields, and sampled columns are generated and applied to a semantic data model automatically. You can change these fields in the properties of the semantic data model. 
{: note}
